...
import { ChevronRight, ChevronDown, FileText, Folder, FolderOpen, RefreshCcw, FilePlus, FolderPlus, MoreHorizontal, ExternalLink, Trash2, Copy, Pencil, CornerDownRight, Download } from "lucide-react"
        ...
        <li key={n.path}>
          {n.type === 'dir' ? (
            <motion.div
              layout
              whileHover={{ x: 2 }}
              className="flex items-center gap-1 cursor-pointer select-none px-2 py-1 rounded hover:bg-muted/50 focus:bg-muted/50 outline-none"
      ...
      {menu && (
        <div className="fixed inset-0 z-50" onClick={closeMenu} onContextMenu={(e) => { e.preventDefault(); closeMenu() }}>
          <div className="absolute bg-popover border shadow-elevation-2 rounded-md py-1 text-sm" style={{ left: menu.x + 2, top: menu.y + 2 }} role="menu">
            ...
            <button className="w-full text-left px-3 py-1.5 hover:bg-muted/50 text-destructive flex items-center gap-2" onClick={() => { removePath(menu.path); closeMenu() }}><Trash2 className="w-4 h-4" /> Delete</button>
          </div>
        </div>
      )}
    </div>
