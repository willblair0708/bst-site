export type DashboardHeader = { org: string; role: string }
export type Kpi = { key: 'time_to_pilot'|'otp_runnable'|'verified_runs'|'irb_cycle'; value: number; unit?: string }
export type MyWork = { repos: Array<{ slug: string; updatedAt: string }>; runs: Array<{ id: string; status: 'queued'|'running'|'success'|'failed'; repo: string; eta_s?: number }> }
export type QueueItem = { id: string; status: 'queued'|'running'|'success'|'failed'; priority: 'low'|'normal'|'high'; cost_est_usd?: number; eta_s?: number }
export type Review = { id: string; repo: string; status: 'open'|'approved'|'changes_requested'; diff: string }
export type Alert = { id: string; type: 'safety'|'data'|'irb'|'policy'; severity: 'low'|'warn'|'high'|'critical'; msg: string; entity_ref?: string; ack?: boolean }
export type Replication = { repo: string; status: 'requested'|'accepted'|'submitted'; due_at?: string }
export type ActivityItem = { id: string; who: string; what: string; repo?: string; at: string }
...
export async function fetchDashboardAggregate(org = 'vousso', role = 'PI'): Promise<DashboardAggregate> {
  // For now, return mocked aggregate in-memory to enable SSR without cross-origin fetch
  const now = new Date()
